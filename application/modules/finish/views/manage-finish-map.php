...
<script>
  ...
  function addpricemap(){
  	...
  	$('select[name="fg[]"]').each(function(){
  	fg_c[m++]  = $(this).val();
  	});
  	$('select[name="fg[]"]').each(function(){
  	fg_m[n++]  = $(this).find("option:selected").text();
  	});
  	var myObject  = new Object();
  	var pa=myObject.part = PartId;
  	var qt=qtyy;
...
<!-- // ends -->
<!-- view finish parts here -->
<div id="modal-view-finish-parts" class="modal fade" tabindex="-1" role="dialog">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        ...
        <div id="resultarea" class="text-center " style="font-size: 15px;color: red;"></div>
        <div class="modal-body">
          <div class="row" id="viewFinishOrder">
          </div>
        </div>
      </div>
      <!-- /.modal-content -->
...
<script>
  ...
  function viewFinishParts(v){
  	var pro=v;
  	var xhttp = new XMLHttpRequest();
  	xhttp.open("GET", "<?=base_url();?>finish/view_finish_parts?ID="+pro, false);
  	xhttp.send();
