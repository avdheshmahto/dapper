...
<div class="col-sm-6" id="invoiceId" >
	...
	<!-- <div class="table-responsive-">
		...
		<table class="table table-striped table-bordered table-hover " >
			<thead>
				<tr>
                    ...
                    <th class="tdcenter">Finish Goods Code</th>
      ...
      <?php
		...
		$productQuery=$this->db->query("select SUM(qty) as qty,part_id from tbl_job_work where order_type='Inspection' group by part_id");
		$i=1;
...
$inbountLogGRNLogQuery=$this->db->query("select SUM(transfer_qty) as rec_qty from tbl_production_order_check where productid='$getProduct->part_id' AND job_order_id = '$lot_no' and order_no='$id'");
            ...
            <td><select name="fg_id[]" id="fg<?=$i;?>" class="form-control">
        
        <option value="">--Select--</option>
        <?php
	
$productQuery=$this->db->query("select *from tbl_product_stock where type='14'");


		
		foreach($productQuery->result() as $getFg){
			
			
		?>
        <option value="<?=$getFg->Product_id?>"><?=$getFg->sku_no;?></option>
        <?php }?>
