...
xhttp.open("GET", "<?=base_url();?>dispatch/view_work_order?ID="+pro, false);
xhttp.send();
...
<h3 class="panel-title" style="float: initial;"><span style="color:#000;">Inspection Details:-<?=$_GET['id'];?></span>
	...
	<a href="<?=base_url();?>dispatch/manage_dispatch_map" class="btn  btn-sm pull-right" type="button"><i class="icon-left-bold"></i> back</a>
</h3>
</div>
...
<li class="active"><a href="#home" data-toggle="tab">Dispatch</a></li>
...
<li style="display:none1;"><a href="#store" data-toggle="tab">Store</a></li>
...
$queryData=$this->db->query("select *from tbl_production_dispatch group by job_order_id");
...
<tbody>
<?php
$poquery=$this->db->query("select * from tbl_product_stock where status='A' and type='14' ");
foreach($poquery->result() as $getPo){
	####### get product #######
	
		$productStockQuery=$this->db->query("select * from tbl_product_serial where product_id='$getPo->Product_id'");
...
<th><?=$getPo->productname;?> & <?=$getPo->sku_no;?></th>
<th><?=$getProductUOM->keyvalue;?></th>
<th><?=$getPo->quantity;?></th>
...
<h4 class="modal-title">Dispatch(Lot No.:-<?=$getsched->lot_no;?>)</h4>
...
<div class="modal-header">
       ...
       <table class="table table-bordered table-hover" >
       	  ...
       	  <tbody id="quotationTable">
       	  	<?php 
			$queryFg=$this->db->query("select *from tbl_production_order_check where lot_no='".$_GET['id']."' and order_type='Inspection'");
            ...
            ?>
				...
				<td><input type ="hidden" name="qty[]" value="<?=$getFG->transfer_qty;?>"><?=$getFG->transfer_qty;?></td>
...
<div id="resultarea" class="text-center " style="font-size: 15px;color: red;"></div> 
<div class="modal-body">
<div class="row" id="viewTransferOrder">
