...
class issueMatiral extends my_controller {
function __construct(){
   parent::__construct();
   $this->load->model('model_inbound_order');
}     

public function issueMatrial(){
	if($this->session->userdata('is_logged_in')){
	  $this->load->view('add-outmatrial');
	}
	else
	{
	  redirect('index');
...
public function getPoItem()
{
    ...
    echo '<label for="po_order">Production No:</label>';
    echo '<select name="po_order" required id="po_order" class="select2 form-control" onchange="getPO();" >';
    echo '<option value = "">--select--</option>';
	$contQuery = $this->db->query("select * from tbl_quotation_purchase_order_hdr where status='A' AND vendor_id = $idd");
	foreach($contQuery->result() as $contRow)
	{
	?>
	<option value = "<?=$contRow->purchaseid;?>">
	<?=$contRow->purchase_no;?></option>
<?php 
  }
  echo '</select>'; 
}
