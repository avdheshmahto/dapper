...
public function stockqty($finishgood_product,$qty)
{
	...
	$qry  = $this->db->query("select S.Product_id,T.quantity from tbl_template_hdr 	H,tbl_template_dtl T,tbl_product_stock S where H.templateid = T.templatehdr AND S.Product_id = T.product_id AND H.product_id =? AND S.type = ?",array(	$finishgood_product,15));
...
{
   $this->db->query("update tbl_product_stock set quantity=quantity+$qty where Product_id=?",array($Pid));
}
