...
<tbody>
<?php
$poquery=$this->db->query("select *from tbl_purchase_order_production_hdr where status='A' and production_id='".$_GET['id']."'");
	...
	<div class="modal-dialog modal-lg">
		<div class="modal-content" >
		   ...
		   <button type="button" class="close" data-dismiss="modal" aria-label="Close" ><span aria-hidden="true">&times;</span></button>
			...
			<div id="Poresultarea" class="text-center " style="font-size: 15px;color: red;"></div> 
...
<div class="col-sm-4"> 
    <input name="date" type="date" value="" class="form-control" > 
           ...
           <div class="col-sm-4"> 
		      ...
		      <input type="hidden" class="form-control input-sm" value="" id="purmproductname"> 
		      <input type="hidden"  class="form-control" value="" id="purmproductid" >
              <label class="control-label">Product Name:</label> <br>
              <?php
			   $selectIssuematQuery=$this->db->query("select *from tbl_issuematrial_dtl where inboundrhdr='".$_GET['id']."'");
			  ...
			  $mQuery=$this->db->query("select *from tbl_machine where code in($issueData)");
			  foreach($mQuery->result() as $getM){
			  ...
			  ?>
              <select id="purprodetails"  class="select2 form-control" onchange="selectListdataPurchase(this);">
              <option value="" selected disabled> --Select-- </option>
              <?php
			  
	          $contQuery=$this->db->query("select distinct(part_id) from tbl_shape_part_mapping where product_id in ($dataMac) ");
              foreach($contQuery->result() as $dt)
              {
				  $productNameQuery=$this->db->query("select *from tbl_product_stock where Product_id='$dt->part_id'");
				  $getProduct=$productNameQuery->row();
				  
                 $prodId   = $getProduct->Product_id;
                 $skuName = $getProduct->sku_no;
				 $prodName = $getProduct->productname;
                 $uom      = $getProduct->usageunit;
              ?>
               <option value="<?=$prodId;?>^<?=$skuName;?>^<?=$uom;?>" ><?=$skuName;?></option>
             <?php } ?>
             <?php
             $mQuery=$this->db->query("select distinct(machine_name) from tbl_machine where code in($issueData)");
			  foreach($mQuery->result() as $getM){
				$productNameQuery=$this->db->query("select *from tbl_product_stock where Product_id='$getM->machine_name'");
				  $getProduct=$productNameQuery->row();
          ...
          <label class="control-label">Usage Unit:</label> 
            <!-- <input type="text" class="form-control input-sm" value="" > -->
               ...
               <select name="unit"  class="form-control" id="Umuom" disabled>
			       <option value="" >----Select Unit----</option>
				    <?php 
						$sqlunit=$this->db->query("select * from tbl_master_data where param_id=16");
						foreach ($sqlunit->result() as $fetchunit){
          ...
          <input type="text" class="form-control" value="" id="purmPrice" >
		   
          <input type="hidden" name = "partid" class="form-control" value="" id="purpartid">
          <input type="hidden" name = "itemid" class="form-control" value="" id="puritemid">
          <input type="hidden" name = "mapType" class="form-control" value="" id="purmapType">
          </div>
           <div class="col-sm-1" > 
           	
           	 <button  style = "margin-top: 20px;" class="btn btn-default"  type="button" onclick="addpricemapPo()"><img src="<?=base_url();?>assets/images/plus.png" />
           	</button>
           </div>
        </div>
    </div>
...
function submitProductionPurchase() {
  ...
  $.ajax({
      url: "productionModule/insert_po_production",
      type: "POST",
      data: form_data,
      processData: false,  // tell jQuery not to process the data
      contentType: false   // tell jQuery not to set contentType
