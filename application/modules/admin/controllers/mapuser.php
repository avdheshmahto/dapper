...
class Mapuser extends my_controller
{
    ...
    public function userroll_list()
    {
        ...
        $res = $this->db->select('*')->where('status', 'A')->get('tbl_user_role_mst');
        
        $i = '0';
        
        foreach ($res->result() as $row) {
            
            $compQuery = $this->db->select('*')->where('user_id', $row->user_id)->get('tbl_user_mst');
            $compRow   = $compQuery->row();
            
            $zoneQuery = $this->db->select('*')->where('role_id', $row->role_id)->get('tbl_role_mst');
