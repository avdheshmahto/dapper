...
class Branch extends my_controller
{
    ...
    public function branch_list()
    {
        ...
        $res = $this->db->select('*')->where('status', 'A')->get('tbl_branch_mst');
        
        $i = '0';
        
        foreach ($res->result() as $row) {
            
            $compQuery = $this->db->select('*')->where('status', 'A', 'comp_id', $row->comp_id)->get('tbl_enterprise_mst');
            $compRow   = $compQuery->row();
            
            $zoneQuery = $this->db->select('*')->where('status', 'A', 'zone_id', $row->zone_id)->get('tbl_region_mst');
