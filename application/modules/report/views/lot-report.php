  ...
  <div class="col-lg-12">
    <div class="panel panel-default">
      <div class="panel-heading clearfix">
        <h4 class="panel-title">LOT REPORT </h4>
          ...
          <div class="form-group panel-body-to">
            ...
            <div class="col-sm-3">
              <select name="contact_id" class="form-control">
                ...
                <?php
                  $cusQuery=$this->db->query("select *from tbl_contact_m where group_name='4'");
                  foreach($cusQuery->result() as $getCust){
                  ?>
                <option value="<?=$getCust->contact_id;?>"><?=$getCust->first_name;?></option>
                <?php }?>
              </select>
            </div>
          </div>
          ...
          <table class="table table-striped table-bordered table-hover dataTables-example1" id="tblData" >
            ...
            <tbody id="getDataTable">
              ...
              <tr class="gradeC record">
                <th><?=$rows->lot_no;?></th>
                <th><?=$rows->invoice_date;?></th>
                <?php
                  $id=$rows->product_id;
                  ?>
                <th><?php 
                  $cumtomerQuery=$this->db->query("select *from tbl_contact_m where contact_id='$rows->vendor_id'");
                  $getCustName=$cumtomerQuery->row();
                  
                  ?>
                  <?=$getCustName->first_name;?>
                </th>
                <th><?=$rows->sub_total?></th>
                <th>
                  <button class="btn btn-default" onClick="openpopup('<?=base_url();?>addproduction/edit_purchase_order_1',1400,600,'view',<?=$rows->purchaseid;?>)" type="button" data-toggle="modal" data-target="#modal-<?php echo $i; ?>"> <i class="fa fa-eye"></i></button>
                </th>
              </tr>
              <?php } //} ?>
            </tbody>
