        ...
        <ul class="panel-tool-options">
          <li>
            <a data-rel="reload" href="#">
              <!--<i class="icon-arrows-ccw"></i>--><i class="fa fa-refresh fa-2x" aria-hidden="true" style="color:black;"></i>
            </a>
          </li>
        </ul>
      </div>
        ...
        <div class="col-sm-12">
          <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper form-inline dt-bootstrap no-footer">
            ...
            <div class="dataTables_length" id="DataTables_Table_0_length">
              <label>
                &nbsp;&nbsp; Show
                <select name="DataTables_Table_0_length" aria-controls="DataTables_Table_0" id="entries"  url="<?=base_url('/report/Report/searchPaymentReceivedReport');?>" class="form-control input-sm">
          ...
          <table class="table table-striped table-bordered table-hover dataTables-example1" id="tblData" >
            ...
            <tbody id="getDataTable">
              ...
              <tr class="gradeC record">
                ...
                <th><?php 
                  $total_bill=0;
                  $contQuery=$this->db->query("select * from tbl_invoice_payment where contact_id='$rows->contact_id' and status='invoice'");
                  foreach($contQuery->result() as $line1)
                  {
                  ...
                  echo $total_bill; ?></th>
                <th><?php 
                  $receive_bill_amount=0;
                  $queryy123=$this->db->query("select * from tbl_invoice_payment where status='PaymentReceived' and contact_id='$rows->contact_id'");
                  foreach($queryy123->result() as $line123)
                  {
...
<script>
  function exportTableToExcel(tableID, filename = ''){
    ...
    //alert();
      ...
      // Specify file name
      filename = filename?filename+'.xls':'PaymentReceivedReport_<?php echo date('d-m-Y');?>.xls';
