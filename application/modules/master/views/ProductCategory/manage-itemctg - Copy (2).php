...
<li class="active"><strong style="font-size:14px;margin-left: 10px;">Manage Product Group </strong> </li> 
</ol>
...
<div id="DataTables_Table_0_wrapper" class="dataTables_wrapper form-inline dt-bootstrap no-footer" style="margin-top: 10px;">
...
<a class="btn btn-sm" data-toggle="modal" formid = "#formId" data-target="#modal-1" id="formreset" style="margin-top: -18px;"><i class="fa fa-arrow-circle-left" onclick="inputdisable();" ></i> Add Category</a>
<a class="btn btn-secondary btn-sm delete_all" style="margin-top: -18px;"><span><i class="fa fa-trash-o"></i> Delete</span></a>
</div>
</div>
...
<?php if($view!=''){ ?>
<button class="btn btn-default modalEditItem" property = "view" type="button" data-toggle="modal" data-target="#modal-1" data-backdrop='static'  arrt = "<?=$rows['name'];?>" cat_id ="<?=$rows['parent_id'];?>" onclick ="editRowCategory(this.id,this);"  id="<?=$rows['id'];?>" data-keyboard='false'> <i class="fa fa-eye"></i> </button>

<?php } if($edit==''){ ?>  
<a  id="<?=$rows['id'];?>" property ="edit"  arrt = "<?=$rows['name'];?>" cat_id ="<?=$rows['parent_id'];?>" onclick ="editRowCategory(this.id,this);" class="btn btn-default modalEditItem"  data-toggle="modal" data-target="#modal-1" >&nbsp; <i class="icon-pencil"></i> &nbsp; </a> 
		  ...
		  <!-- <div class="form-group">
			...
			<div class="col-sm-6"> 
				<select class=" form-control" required name="type" id="type"> 
					...
					<option value="3" class="listClass">Scope </option>
				</select>
			</div>
		  </div> -->
		  <!-- <div class="form-group">
