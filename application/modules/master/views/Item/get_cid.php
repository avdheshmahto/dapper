...
<div class="form-group" id="consigneeMapping" style="display:none"> 
				   <div class="col-sm-12" > 
				      	...
				      	<select  class="select2 form-control" id="entity" >
						    <option value="">----Select ----</option>
							  <?php 
								...
								foreach ($sqlprotype->result() as $fetch_protype){
							    ?>
						        <option value="<?=$fetch_protype->Product_id;?>"><?=$fetch_protype->productname; ?></option>
							 <?php } ?>
				        </select> 
				      </td>
				   <!--<td>
                    ...
                    <?php
					$queryMap=$this->db->query("select *from tbl_shape_part_mapping");
					foreach($queryMap->result() as $getMap){
						$queryProduct=$this->db->query("select *from tbl_product_stock where Product_id='$getMap->part_id'");
						$getProduct=$queryProduct->row();
					?>
                    <tr><td><input  type ="hidden" class="form-control" name="entity[]" value="<?=$getMap->product_id?>">
                    <input   type ="text" readonly class="form-control"  value="<?=$getProduct->productname;?>"></td>
                    <td><i class="fa fa-trash  fa-2x" id="quotationdel" style="font-size:20px;" attrval="6" aria-hidden="true"></i></td>
                    </tr>
<?php }?>
                    </tbody>
				 </table>
